...
class RingModel:
    ...
    def enter_ring(self, boxer_id: int):
        ...
        current_names = [Boxers.get_boxer_by_id(b).name for b in self.ring]
        logger.info(f"Current boxers in the ring: {current_names}")
    ...
    def get_boxers(self) -> List[Boxers]:
        ...
        for boxer_id in self.ring:
            # If a boxer is in the cache and the TTL is still valid, use the cached value.
            if boxer_id in self._ttl and current_time < self._ttl[boxer_id]:
                ...
                try:
                    boxer = Boxers.get_boxer_by_id(boxer_id)
                    self._boxer_cache[boxer_id] = boxer
                    self._ttl[boxer_id] = time.time()
                except ValueError:
                    logger.error(f"Boxer with ID {boxer_id} not found in DB.")
                    continue
