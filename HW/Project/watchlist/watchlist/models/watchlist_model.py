...
from watchlist.models.song_model import Songs
...
class WatchlistModel:
    ...
    def add_song_to_watchlist(self, song_id: int) -> None:
        """
        ...
        logger.info(f"Successfully added to playlist: {song.director} - {song.title} ({song.year})")
    ...
    def get_song_by_song_id(self, song_id: int) -> Songs:
        ...
        song = self._get_song_from_cache_or_db(song_id)
        logger.info(f"Successfully retrieved song: {song.director} - {song.title} ({song.year})")
        return song
